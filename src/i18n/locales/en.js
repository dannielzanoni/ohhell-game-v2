export const en = {
  auth: {
    avatarModalTitle: 'Select your avatar',
    avatars: 'Avatars',
    closeModal: 'Close modal',
    editGuestAvatar: 'Edit guest avatar',
    googleKeepsGuest: 'Signing in with Google will keep your guest username and avatar.',
    googleLoginError: 'Google login failed. Please try again.',
    googleUnavailable: 'Google login is unavailable right now.',
    loginGoogle: 'Login with Google',
    logoutGoogle: 'Logout',
    logoutGoogleAria: 'Logout from Google',
    nick: 'Nick',
    nickPlaceholder: 'Enter your nick',
    or: 'or',
    playAsGuest: 'Play as Guest',
    requiredDescription: 'Login as guest or with Google to continue.',
    requiredTitle: 'Login required',
    saveNick: 'Save nick',
    saveProfileError: 'Could not save the profile.',
    selectAvatar: 'Select your avatar',
    selectAvatarAria: 'Select avatar {{label}}',
  },
  common: {
    appName: 'Oh Hell',
    backHome: 'Back to Home',
    close: 'Close',
    create: 'Create',
    home: 'Home',
    join: 'Join',
    link: 'Link',
    loading: 'Loading...',
    play: 'Play',
    refresh: 'Refresh',
    settings: 'Settings',
    appNameShort: 'Oh Hell',
  },
  game: {
    authSaveGuest: 'Save your guest before entering the room.',
    bidDone: 'Bid {{number}} done',
    bidError: 'Could not send the bid.',
    bidPending: 'Bid {{number}} pending',
    chosenBid: 'Chosen bid',
    confirmProfile: 'Confirm your nick and avatar before entering the lobby.',
    confirmProfileError: 'Could not confirm the profile.',
    connectionNotReady: 'The match connection is not ready yet.',
    copyRoomLink: 'Copy room link',
    deckAndJoker: 'Deck and joker card',
    dragToTarget: 'Drag to target',
    enterRoom: 'Enter room',
    enterRoomError: 'Could not enter the room.',
    enteringRoom: 'Entering...',
    extraPointDone: 'Extra point {{number}} done',
    backToMenu: 'Back to Menu',
    everyoneRanOut: 'Everyone ran out of lives.',
    gameEnded: 'Game ended',
    lastPlayerStanding: 'Last player standing.',
    lifeLossEvent: 'Jo\u00e3o plays',
    selectedDeckLabel: 'Selected deck',
    lostLives_one: 'Lost {{count}} life',
    lostLives_other: 'Lost {{count}} lives',
    noWinners: 'No winners',
    winner: 'Winner',
    winners: 'Winners',
    lives_one: '{{count}} life',
    lives_other: '{{count}} lives',
    missingAuth: 'Register your guest before entering the room.',
    lifeMultiplierDisplay: 'Life multiplier: {{multiplier}}',
    powerLobbySettings: 'Power Lobby Settings',
    playCardError: 'Could not play the card.',
    powerCard: 'Power card',
    powerCardDragToTarget: 'Drag this power card onto a player avatar.',
    powerCardTargetError: 'No valid target for this power card.',
    powerCardUseError: 'Could not use the power card.',
    mana: 'Mana',
    manaValue: '{{current}}/{{max}} mana',
    playersReady: '{{readyCount}}/{{totalPlayers}} players ready',
    ready: 'Ready',
    readyError: 'Could not update ready state.',
    readySending: 'Sending...',
    reconnecting: 'Reconnecting to the match...',
    roomLink: 'Room link',
    chooseMercenaryDescription:
      'Select a contract before entering this Hell Hand table.',
    chooseMercenaryTitle: 'Choose your mercenary',
    enterWithMercenary: 'Enter',
    noMercenariesAvailable: 'No mercenaries are available to enter the room.',
    selectMercenaryTitle: 'Select a mercenary before readying up',
    socketError: 'Room realtime connection error.',
    tableAria: 'Oh Hell game table',
    timerBid: 'Bid',
    timerCards: 'Cards',
    timerPower: 'Power',
    timerPlay: 'Play',
    waitDeal: 'Wait for the play phase to play a card.',
    waitPower: 'Wait for the power phase to use a power card.',
    waiting: 'Waiting',
    waitingForPlayersTitle: 'Waiting for at least 2 players',
    waitTurn: 'Wait for your turn.',
    skipPowerPhase: 'Skip power phase',
  },
  gameTypes: {
    eyebrow: 'Game type',
    fodinhaClassic: 'Fodinha Classic',
    fodinhaClassicDescription: 'The current Oh Hell/Fodinha trick-taking rules.',
    fodinhaPower: 'Hell Hand',
    fodinhaPowerDescription:
      'A dark-magic mode with 50 lives, heavier damage, and Lua-powered cards.',
  },
  leaderboard: {
    averageBid: 'Avg bid',
    bid: 'Bid',
    bidHit: 'Bid hit',
    emptyDescription: 'Finish matches to appear on the leaderboard.',
    emptyTitle: 'No data yet',
    eyebrow: 'Global statistics',
    favorite: 'Favorite',
    games: 'Games',
    loadError: 'Could not load the leaderboard.',
    noRounds: 'No rounds',
    player: 'Player',
    rounds: 'Rounds',
    title: 'Leaderboard',
    trumps: 'Trumps',
    winRate: 'Win rate',
    wins: 'Wins',
    winsShort: 'wins',
  },
  nav: {
    closeMenu: 'Close menu',
    darkTheme: 'Dark theme',
    expandMenu: 'Expand menu',
    gameHub: 'Game hub',
    languageTitle: 'Language: {{language}}',
    lightTheme: 'Light theme',
    minimizeMenu: 'Minimize menu',
    toggleTheme: 'Toggle theme',
  },
  pages: {
    createGame: {
      configureBefore: 'Configure the match before entering the table.',
      configurations: 'Game Configurations',
      createError: 'Could not create the room.',
      creating: 'Creating...',
      communityDeckConsentHint:
        'I understand this match will use player-created cards and balance, and everyone joining the lobby will play with this exact community deck.',
      communityDeckConsentLabel: 'Confirm community deck use',
      communityDeckConsentRequired:
        'Confirm the community deck warning before creating the room.',
      communityDeckWarningDescription:
        'Community decks can include experimental scripts, unusual balance, and player-authored card pools. Use them intentionally.',
      communityDeckWarningTitle: 'Community Deck Warning',
      enablePublicRoom: 'Enable public room',
      lifeMultiplier: 'Life multiplier',
      lifeMultiplierError: 'Enter a life multiplier greater than 0.',
      lifeMultiplierHint:
        'Each mercenary keeps its own base life, and this multiplier is applied to that value for everyone in the lobby.',
      liveTable: 'Live table',
      loadingPowerDecks: 'Loading power decks...',
      livesNumber: 'Number of Lives',
      livesRangeError: 'Enter a whole number from {{min}} to {{max}}.',
      livesRangeHint:
        'Allowed range: {{min}}-{{max}}. Round damage: {{roundDamage}}.',
      noOptions: 'No options found.',
      powerDeck: 'Fodinha Power deck',
      powerDeckEmpty: 'No power decks are available yet. Create a deck before starting Fodinha Power.',
      powerDeckHint:
        'Everyone in the lobby plays with the exact deck selected by the lobby creator.',
      powerDeckCardCount: '{{count}} cards',
      powerDeckCreatedBy: 'By {{name}}',
      powerDeckLoadError: 'Could not load power decks.',
      powerDeckGroupCommunity: 'Community decks',
      powerDeckGroupCommunityHint:
        'Player-built decks with their own card mixes, kept separate so you know exactly when you are opting into community content.',
      powerDeckGroupOfficial: 'Official decks',
      powerDeckGroupOfficialHint:
        'Curated decks maintained as the default competitive baseline for Fodinha Power.',
      powerDeckOfficialCreator: 'Official',
      powerDeckOption: '{{name}} ({{count}} cards)',
      powerDeckSelected: 'Selected',
      powerLivesDefinedByMercenary: 'Lives are defined by the selected mercenary',
      powerLivesDefinedByMercenaryHint:
        'Fodinha Power base life and initial mana now come from the mercenary Lua script instead of the lobby form.',
      powerDeckRequired: 'Select a Fodinha Power deck.',
      publicRoom: 'Public room',
      selectPowerDeck: 'Select a deck',
    },
    communityCards: {
      create: 'Create card',
      createdBy: 'By {{name}}',
      description:
        'Community Fodinha Power cards are uploaded from the playground with artwork and Lua effects.',
      emptyDescription: 'Publish a card from the playground to start the shared deck.',
      emptyTitle: 'No community cards yet',
      eyebrow: 'Community decks',
      kind: {
        community: 'Community',
        official: 'Official',
      },
      life: '{{count}} life',
      loadError: 'Could not load community cards.',
      title: 'Community Cards',
    },
    github: {
      description: 'Project repository for Oh Hell Game v2',
      eyebrow: 'Repository',
      open: 'Open repository',
      title: 'GitHub',
    },
    home: {
      stats: {
        empty: 'Play matches to build your stats.',
        eyebrow: 'Your stats',
        loadError: 'Could not load your stats.',
        title: 'Player stats',
      },
      tagline:
        'Fodinha, also known as Oh Hell, is a classic trick-taking card game. Predict your wins, outsmart your friends, and have fun online!',
    },
    gameModeSelect: {
      eyebrow: 'Game modes',
      title: 'Choose your table',
      classic: {
        description: 'The classic bid-and-trick mode with clean rules and fast rounds.',
        title: 'Fodinha Classic',
      },
      hellHand: {
        description: 'A darker table of mercenaries, curses, power cards, and desperate bets.',
        title: 'Hell Hand',
      },
    },
    hellHandHome: {
      actions: {
        cards: 'Cards',
        howToPlay: 'How To Play',
        mercenaries: 'Mercenaries',
        play: 'Play',
        rooms: 'Rooms',
        settings: 'Settings',
        workshop: 'Workshop',
      },
      description:'Mercenaries bargain with cursed decks, blood debts, and impossible bids beneath a blackened table.',
      luaIntegration: ' Create community cards and decks with Lua integration.',
      eyebrow: 'Dark magic mode',
      pillOne: 'Cursed bids',
      pillTwo: 'Mercenary decks',
      title: 'Hell Hand',
    },
    hellHandWorkshop: {
      apiDocumentation: 'API Documentation',
      apiDocumentationDescription: 'Backend endpoints and contracts',
      cards: 'Cards',
      cardsDescription: 'Official and Community Cards',
      communityCards: 'Community Cards',
      communityCardsDescription: 'Cards created by the community and published through Playground.',
      description: 'Forge power cards and decks for Hell Hand tables.',
      eyebrow: 'Arcane workshop',
      officialCards: 'Official Cards',
      officialCardsDescription: 'Official cards ready to compose decks and matches.',
      open: 'Open',
      playground: 'Playground',
      playgroundDescription: 'Create your card',
      powerDecks: 'Power Decks',
      powerDecksDescription: 'Deck Builder',
      title: 'Workshop',
    },
    hellHandGame: {
      characterEyebrow: 'Mercenary',
      characterRequired: 'Select a mercenary before playing.',
      characterTitle: 'Choose your Mercenary',
      eyebrow: 'Table contract',
      selectCharacter: 'Select',
      title: 'Prepare Hell Hand',
      unlockCharacter: 'Change',
    },
    howToPlay: {
      description: 'Rules, rounds, bids, trump cards, and card strength.',
      title: 'How To Play',
      rules: {
        cardAlt: '{{label}} - 8-bit card',
        cardsText: 'The deck contains 40 cards and is shuffled at the beginning of every round. In the first round, each player receives 1 card; the amount increases in the following rounds. One card is turned face up in the center of the table to define the trump rank.',
        cardsTitle: 'Cards',
        intro1: 'Fodinha can be played by 2 to 10 players. The last player remaining with at least one life wins the game.',
        intro2: 'Each participant starts with 1 to 5 lives and loses one or more whenever their bid is incorrect.',
        intro3: 'The number of cards dealt to each player increases every round. The goal is to predict how many tricks you will win during that round.',
        intro4: 'If your prediction is wrong, you lose the difference between your bid and the number of tricks you actually won. For example, if you bid 1 but win 2 tricks, you lose 1 life.',
        jokerText: 'The trump rank is always one position above the face-up card. For example: face-up card 4, trump rank 5.',
        jokerTitle: 'Trump Card',
        rankOrder: 'Ranks, from weakest to strongest',
        strengthTitle: 'Card and suit strength',
        suitOrder: 'Suits, from weakest to strongest',
        suits: { cups: 'Cups', golds: 'Golds', clubs: 'Clubs', swords: 'Swords' },
        bidsText: 'After all players receive their cards, the bidding phase begins. The sum of all bids cannot equal the number of cards dealt to each player. This guarantees that at least one participant will lose life every round.',
        bidsTitle: 'Bids',
      },
    },
    characters: {
      backToMercenaries: 'Back to mercenaries',
      cardLabel: 'Mercenary',
      cardsEyebrow: 'Mercenary cards',
      cardsTitle: 'Featured cards',
      cardInfo: 'View information for {{name}}',
      cardStory: 'Card story',
      carouselEyebrow: 'Selection',
      carouselTitle: 'Mercenary carousel',
      chooseCharacter: 'Select {{name}}',
      description:
        'Choose a mercenary to inspect their cards and abilities.',
      edit: 'Edit',
      eyebrow: 'Mercenaries',
      emptyCardsDescription:
        'When this mercenary has card images and data, they will appear here.',
      emptyCardsTitle: 'Cards not added yet',
      historyEyebrow: 'Story',
      inspect: 'Inspect',
      manaCost: 'Mana cost',
      loadError: 'Could not load backend mercenaries.',
      next: 'Next mercenary',
      passiveScript: 'Passive Lua script',
      previous: 'Previous mercenary',
      statsEyebrow: 'Traits',
      statsTitle: 'Play profile',
      stats: {
        style: 'Style',
        temper: 'Temper',
      },
      title: 'Mercenaries',
      admin: {
        banner: 'Banner image',
        bannerRequired: 'Select a banner image.',
        createError: 'Could not save the mercenary.',
        createNew: 'Create new',
        description: 'Description',
        editExisting: 'Edit existing mercenary',
        editing: 'Editing {{name}}',
        editTitle: 'Edit mercenary',
        eyebrow: 'Admin tools',
        icon: 'Icon image',
        iconRequired: 'Select an icon image.',
        imageOptional: '(optional; keeps the current image)',
        id: 'Mercenary ID',
        luaFetchError: 'Could not load the latest Lua editor source.',
        name: 'Name',
        nameRequired: 'Enter a mercenary name.',
        passiveRequired: 'Enter a passive Lua script.',
        luaValidationRequired: 'Wait for Lua validation to finish successfully before saving.',
        passiveScript: 'Passive Lua script',
        save: 'Save mercenary',
        saving: 'Saving...',
        subtitle: 'Subtitle',
        title: 'Create mercenary',
        newMercenary: 'New mercenary',
        update: 'Update mercenary',
        updateError: 'Could not update the mercenary.',
      },
      items: {
        artemis: {
          cards: {
            madJustice: {
              description:
                'When low on life, Artemis turns defense into card draw and pressures the end of the round.',
              story:
                'The last watch began when Artemis kept guard long after the night should have been lost.',
              title: 'Mad Justice',
            },
            bloodTransfusion: {
              description:
                'Prevents life loss and rewards careful predictions with an extra card.',
              story:
                'They say this guard only appears to those who know how to wait for the right moment.',
              title: 'Blood Transfusion',
            },
            deepRed: {
              description:
                'Every player that did not lost life this turn, takes 10 damage. Draw a magic card for each player that did not lose life this way.',
              story:
                'The silver oath was sworn before a cold blade and has never been broken.',
              title: 'Deep Red',
            },
            signInBlood: {
              description:
                'Target a player to lose 5 life points and draw 3 cards.',
              story:
                'The forest signs guide Artemis when every path at the table seems closed.',
              title: 'Sign in Blood',
            },
            hunterGrace: {
              description:
                'Lets Artemis reposition her plan without losing control over the table tempo.',
              story:
                'The hunter’s grace is quiet: by the time the target notices, the round has already turned.',
              title: "Hunter's Grace",
            },
          },
          history:
            'Artemis guards the old routes around the table and plays as if she read the round before it began. Her story is built on vows, protection, and precise choices: she prefers to win through control, holding risk until the perfect window appears.',
          style: 'Control',
          subtitle: 'Guardian of vows and the last light of the round.',
          temper: 'Patient',
          title: 'Artemis',
        },
        carmen: {
          cards: {
            carmenCard2: {
              description:
                'An opening card for Carmen to control the table tempo and prepare the next play.',
              story:
                'Carmen learned early that every debt has its own melody.',
              title: 'Marked Tempo',
            },
            carmenCard4: {
              description:
                'Pressures players who bid high and rewards Carmen when the table hesitates.',
              story:
                'When Carmen smiles, someone has already accepted a contract without noticing.',
              title: 'Crimson Contract',
            },
            carmenCard5: {
              description:
                'Turns a dangerous round into advantage by punishing poorly calculated losses.',
              story:
                'The final chord always arrives after the table believes the music is over.',
              title: 'Final Chord',
            },
          },
          history:
            'Carmen negotiates with cold blood and plays as if she knows the exact price of every promise. She enters the table to collect debts, twist bids, and turn small hesitations into advantage.',
          style: 'Pressure',
          subtitle: 'A pact collector who turns promises into debts.',
          temper: 'Calculating',
          title: 'Carmen',
        },
        conjuruz: {
          cards: {},
          history:
            'Conjuruz is still a mercenary surrounded by mystery. His banner already hints at an arcane presence, but his cards will be revealed when his deck is ready.',
          style: 'Conjuration',
          subtitle: 'A summoner of old signs and unstable pacts.',
          temper: 'Enigmatic',
          title: 'Conjuruz',
        },
        gambler: {
          cards: {
            crossYourFingers: {
              description:
                'Raises the reward for a risky prediction, but costs life when the bet fails.',
              story:
                'All In is the card the Gambler smiles before revealing, even when the whole table knows the risk.',
              title: 'All In',
            },
            isRightfullyMine: {
              description:
                'At the end of the round, draw one card for each bid you hit and discard one card for each bid you missed.',
              story:
                'His last coin has bought escapes, impossible wins, and a few debts that are still breathing.',
              title: 'Is Rightfully Mine',
            },
            giveItToMe: {
              description:
                'Manipulates a short clash and creates advantage when the player accepts calculated risk.',
              story:
                'The loaded dice never land by chance; they land where the Gambler prepared the story to end.',
              title: 'Give It To Me',
            },
            allIn: {
              description:
                'Manipulates a short clash and creates advantage when the player accepts calculated risk.',
              story:
                'The loaded dice never land by chance; they land where the Gambler prepared the story to end.',
              title: 'Loaded Dice',
            },
            guabiru: {
              description:
                'Target player gets +2 to bids this turn.',
              story:
                '',
              title: 'Guabiru',
            },
          },
          history:
            'The Gambler learned to hear the sound of cards before placing a bet. He lives between bluff and courage, selling the idea of chaos while calculating every possible loss. When he sits at the table, every round seems to hide a second wager.',
          style: 'Risk',
          subtitle: 'Life isn\'t always a matter of good cards. Sometimes it\'s a good play with a bad hand.',
          temper: 'Bold',
          title: 'Gambler',
        },
        leandro: {
          cards: {},
          history: 'Shortly after beginning his training in the army, Leandro drew the attention not only of the general, but also of scholars, mages, and even Fate itself.',
          style: 'Mage',
          subtitle: 'A prodigy noticed by generals, scholars, mages, and Fate itself.',
          temper: 'Enigmatic',
          title: 'Leandro',
        },
      },
    },
    links: {
      createGame: {
        description: 'Create a game and invite your friends to play.',
        label: 'Create a Game',
      },
      github: {
        description: 'Project repository for Oh Hell Game v2',
        label: 'GitHub',
      },
      howToPlay: {
        description: 'Rules and game guide.',
        label: 'How To Play',
      },
      powerDecks: {
        description: 'Create decks from power cards.',
        label: 'Power Decks',
      },
      leaderboard: {
        description: 'Player ranking.',
        label: 'Leaderboard',
      },
      characters: {
        description: 'View mercenary profiles and cards.',
        label: 'Mercenaries',
      },
      playground: {
        description: 'Create and save custom cards.',
        label: 'Playground',
      },
      rooms: {
        description: 'View and enter open rooms.',
        label: 'Rooms',
      },
    },
    playground: {
      assetImage: 'Inner card asset',
      cardData: 'Card data',
      clear: 'Clear form',
      description:
        'Build cards with a separate frame template, import an inner asset, tune position and text, then save them to the shared backend catalog.',
      displayOnCard: 'Display',
      drafting: 'Drafting',
      emptySavedDescription: 'Fill the form and save the first backend card.',
      emptySavedTitle: 'No cards available',
      editCard: 'Edit card',
      eyebrow: 'Card playground',
      export: 'Export JSON',
      cardTypes: {
        instant: 'Instant',
        interactive: 'Interactive',
        targetable: 'Targetable',
      },
      fields: {
        cardType: 'Card type',
        description: 'Description',
        manaCost: 'Mana cost',
        quantity: 'Deck quantity',
        title: 'Title',
      },
      imageError: 'Could not import the image.',
      imageHint:
        'Use Template for the card frame and Asset for artwork positioned by X/Y. PNG, JPG, WEBP, and SVG are accepted.',
      imageLayout: {
        eyebrow: 'Image geometry',
        height: 'Height',
        title: 'Artwork position and size',
      },
      importImage: 'Import image',
      importTemplate: 'Import template',
      loadError: 'Could not load card definitions.',
      luaFetchError: 'Could not load the latest Lua editor source.',
      luaDefinedValue: 'Defined by Lua script',
      luaRequired: 'Enter a Lua script before saving.',
      luaValidationRequired: 'Wait for Lua validation to finish successfully before saving.',
      luaScript: 'Lua effect script',
      layout: {
        case: 'Upper/lowercase',
        caseOptions: {
          lower: 'lowercase',
          none: 'Normal',
          upper: 'UPPERCASE',
        },
        color: 'Color',
        eyebrow: 'Text geometry',
        size: 'Size',
        title: 'Position and size by field',
        width: 'Width',
      },
      newCard: 'New card',
      officialCard: 'Create as official card',
      officialCardHint:
        'Only admin users can save official cards. Other cards are community cards.',
      publish: 'Publish',
      publishError: 'Could not save the community card.',
      publishOfficialSuccess: '{{name}} was saved as an official card.',
      publishing: 'Saving...',
      publishSuccess: '{{name}} was saved as a community card.',
      removeImage: 'Remove image',
      removeLua: 'Remove Lua',
      removeTemplate: 'Remove template',
      renderError: 'Could not save the composed image.',
      saveCard: 'Save card',
      saveImage: 'Save PNG',
      savedCards: 'Cards: {{count}}',
      storage: 'Shared card definitions',
      templateError: 'Could not import the template.',
      templateRequired: 'Import a template before saving the card.',
      templateImage: 'Card template',
      title: 'Card generator',
      untitled: 'Untitled card',
      updateCard: 'Update card',
      updateSuccess: '{{name}} was updated.',
      viewCommunityCards: 'Create a power deck',
    },
    powerDecks: {
      bucket: 'Link cards to',
      bucketHint: '{{generic}} table cards. {{mercenary}} linked cards in the active bucket.',
      cardsEyebrow: 'Card definitions',
      cardsTitle: 'Official and community cards',
      create: 'Create deck',
      createError: 'Could not create the deck.',
      createEyebrow: 'Deck builder',
      createTitle: 'Create a Fodinha Power deck',
      createdBy: 'By {{name}}',
      creating: 'Creating...',
      deckCardCount: '{{count}} cards',
      description:
        'Build named Fodinha Power decks from official cards and cards published by the community.',
      emptyCardsDescription: 'Publish cards from the playground before creating decks.',
      emptyCardsTitle: 'No cards available',
      editDeck: 'Edit deck',
      editTitle: 'Edit a Fodinha Power deck',
      eyebrow: 'Fodinha Power',
      fields: {
        description: 'Description',
        name: 'Deck name',
      },
      genericBucket: 'Table deck',
      cardTypes: {
        instant: 'Instant',
        interactive: 'Interactive',
        targetable: 'Targetable',
      },
      kind: {
        community: 'Community',
        official: 'Official',
      },
      loadError: 'Could not load power decks.',
      luaScript: 'Lua effect script',
      manaCost: 'Mana cost',
      nameRequired: 'Enter a deck name.',
      newDeck: 'New deck',
      noDecks: 'No decks created yet.',
      officialCreator: 'Official',
      officialDeck: 'Create as official deck',
      officialDeckHint:
        'Only admin users can save official decks. Other decks are community decks.',
      quantity: 'Deck quantity',
      savedEyebrow: 'Saved decks',
      savedTitle: 'Decks: {{count}}',
      saveAsDraft: 'Save as draft',
      saveAsDraftHint:
        'Drafts are visible only to you and cannot be used to start a match.',
      selectedCards: '{{count}} selected cards',
      selectedHint:
        'Cards are optional. Link each card once to the table deck or to a mercenary bucket. Card quantity comes from the Lua script.',
      status: {
        draft: 'Draft',
        valid: 'Playable',
      },
      title: 'Power Decks',
      update: 'Update deck',
      updateError: 'Could not update the deck.',
      updating: 'Updating...',
    },
    rooms: {
      create: 'Create',
      emptyDescription: 'Create a new table to invite your friends.',
      emptyTitle: 'No open rooms',
      enter: 'Enter',
      eyebrow: 'Live tables',
      loadError: 'Could not load the rooms.',
      players: 'Players',
      regionWaiting: 'BR - Waiting',
      roomInactiveToast: 'The room was closed after 3 minutes without activity.',
      room: 'Room',
      title: 'Rooms',
    },
    settings: {
      description: 'A reserved page for preferences, table options, and account settings.',
      title: 'Settings',
    },
  },
  settings: {
    cardBack: 'Card back',
    cardBackOption: 'Back {{number}}',
    deck: 'Type of deck',
    deckTypeTab: 'Type of deck',
    english: 'English',
    french: 'French',
    generalVolume: 'General volume',
    hellHandHomeMusicVolume: 'Intro music volume',
    language: 'Language',
    languageDescription: 'Choose the app language.',
    portuguese: 'Portuguese',
    sounds: 'Sounds',
    spanish: 'Spanish',
    spanish8Bit: 'Spanish 8-bit',
    title: 'Settings',
  },
};
