...
import { getClassicCardStrength as getCardStrength } from '@/games/classic/model/cardRules.js';
import {
  getClassicCardLabel as getCardLabel,
...
function getStrongestTurn(pile, upcard) {
  return (pile || []).reduce((strongestTurn, turn) => {
    ...
    return getCardStrength(turn.card, upcard) > getCardStrength(strongestTurn.card, upcard)
      ? turn
      : strongestTurn;
  }, null);
}
...
function getLobbyLifes(lobbyId, routeLifes) {
  ...
  if (lobbyId) {
    const savedLifes = Number(localStorage.getItem(`ohhell_lobby_lifes_${lobbyId}`));
...
function createGameEndSummary(lifesByPlayer, playersById, defaultLifes) {
  ...
  const winners = winnerIds.map((playerId) => {
    const existing = playersById[playerId] || createFallbackPlayer(playerId, defaultLifes);
...
function ActionTimer({ onExpire, timer, visualOffsetX = 0, visualOffsetY = 0, visualScale = 1 }) {
  const { t } = useTranslation();
...
function PlayedCardAnimation({ animationDuration, card, cardBackSrc, deckType, onAnimationEnd }) {
  if (!card) {
    return null;
  }
...
function HellHandMercenaryJoinGate({ characters, error, isLoading, onRetry, onSelect, open }) {
  const { t } = useTranslation();
  return (
    <Dialog open={open}>
      <DialogContent
...
export function GameSessionPage({
  ...
  const playConfiguredSound = useCallback((soundSrc) => {
  ...
  const playTurnPromptSound = useCallback(
    (type, playerId) => {
      ...
      playConfiguredSound(
        type === 'bid' ? bidTurnSound : getPlayerTurnSound(gamePreferencesRef.current.turnSound),
      );
    },
    [playConfiguredSound],
  );
