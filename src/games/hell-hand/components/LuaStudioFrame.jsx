...
export function LuaStudioFrame({
  ...
  const updateSnippetId = useCallback((nextSnippetId) => {
    setSnippetId(nextSnippetId);
    onSnippetChange?.(nextSnippetId);
  }, [onSnippetChange]);

  const postSource = useCallback(() => {
  ...
  }, [luaStudioOrigin, onSourceChange, onValidationChange, source, updateSnippetId]);
