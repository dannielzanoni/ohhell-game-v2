  ...
  return (
    <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-black/65 px-3 py-2 text-white shadow-xl backdrop-blur">
      <div className="relative grid size-8 place-items-center">
        <img src={cardBack} alt="" className="absolute size-7 rounded border border-black object-cover opacity-70" draggable="false" />
        <Layers3 className="relative z-10 size-4 text-amber-200" />
      </div>
      <div>
        <span className="block text-[0.58rem] font-black uppercase tracking-[0.16em] text-zinc-400">Deck clássico</span>
...
export function ClassicTableView({
  ...
  const [isMobilePortrait, setIsMobilePortrait] = useState(
    () => Math.min(window.innerWidth, window.innerHeight) < 640 && window.innerHeight >= window.innerWidth,
  );
  ...
  useEffect(() => {
    const updateOrientation = () => setIsMobilePortrait(
      Math.min(window.innerWidth, window.innerHeight) < 640 && window.innerHeight >= window.innerWidth,
    );
    ...
    <div className="relative flex min-h-[calc(100vh-7rem)] flex-col overflow-hidden rounded-[1.75rem] border border-amber-200/15 bg-black shadow-2xl shadow-black/35">
      <div className="relative min-h-0 flex-1 overflow-hidden">
        <div
          ...
          style={{ backgroundImage: `linear-gradient(rgba(0, 0, 0, .2), rgba(0, 0, 0, .35)), url(${gameTableBackground})` }}
        />
        ...
        {orderedPlayers.map((player, index) => (
          <PlayerSeat
            ...
            position={getSeatPosition(index, orderedPlayers.length, player.id === selectedPlayerId, visualConfig.seatOrbitX || 36, visualConfig.seatOrbitY || 28, visualConfig.seatLift ?? 2)}
            visualScale={(visualConfig.seatScale || 1) * tableScale}
          />
        ))}
        <PlayedCardAnimation animationDuration={animationDuration} card={animationCard} cardBackSrc={cardBack} deckType={deckTypes.SPANISH_8BIT} onAnimationEnd={onAnimationEnd} />
        <BidControls onBid={onBid} possibleBids={[0, 1, 2, 3, 4, 5]} visualOffsetX={visualConfig.bidControlOffsetX || 0} visualOffsetY={visualConfig.bidControlOffsetY || 0} visualScale={(visualConfig.bidControlScale || 1) * tableScale} />
      </div>
      <div className="relative h-[14.85rem] shrink-0 bg-zinc-950" style={visualConfig.classicHandAreaHeightVh ? { height: `${visualConfig.classicHandAreaHeightVh}vh` } : undefined}>
        <PlayerHand canPlayCards cardBackSrc={cardBack} cards={currentPlayer?.classicHand || []} centered deckType={deckTypes.SPANISH_8BIT} onPlayCard={onClassicPlay} upcard={upcard} visualContainerHeightVh={visualConfig.classicHandAreaHeightVh || 0} visualContainerWidthVw={visualConfig.classicHandBoxWidthVw || 0} visualContainerOffsetX={visualConfig.classicHandBoxOffsetX || 0} visualContainerOffsetY={visualConfig.classicHandBoxOffsetY || 0} visualOffsetX={visualConfig.classicHandOffsetX || 0} visualOffsetY={visualConfig.classicHandOffsetY || 0} visualScale={(visualConfig.classicHandScale || 1) * tableScale} />
      </div>
    </div>
  );
}
