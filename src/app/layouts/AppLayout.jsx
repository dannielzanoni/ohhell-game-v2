...
function getRouteLobbyGameType(pathname) {
  const match = pathname.match(/^\/classic\/game\/([^/]+)$/);
  return match
...
export function AppLayout() {
  ...
  const isLobbyGameRoute = /^\/classic\/game\/[^/]+$/.test(location.pathname);
  const showNavBar = !isLobbyGameRoute || routeGameType === GAME_TYPES.CLASSIC;
  ...
  useEffect(() => {
    const handleGameTypeChange = (event) => {
      if (
        event.detail?.lobbyId &&
        location.pathname === `/classic/game/${event.detail.lobbyId}`
      ) {
