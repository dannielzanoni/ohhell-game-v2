# PRD — US 022 — Renovação de sessão

**Módulo:** MOD 03 — Autenticação e perfil  
**Status:** Planejado  
...
Esta especificação detalha exclusivamente **US 022 — Renovação de sessão**. Ela deve ser implementada no mesmo aplicativo React/Vite, usando a experiência Mobile abaixo de 768 px e a experiência Desktop a partir de 768 px.
...
Entregar **Renovação de sessão** de forma verificável, sem duplicar regras, estado, chamadas HTTP, sessão WebSocket ou persistência entre os layouts Mobile e Desktop.
...
- **RF-01:** Usa `POST /auth/refresh` antes de recriar o guest.
- **RF-02:** Refresh token é rotacionado e armazenado pelo adaptador.
- **RF-03:** Uma única renovação é executada para requisições concorrentes.
- **RF-04:** Falha definitiva limpa a sessão e pede confirmação do perfil.

## 7. Critérios de aceite

- [ ] **CA-01:** Usa `POST /auth/refresh` antes de recriar o guest.
- [ ] **CA-02:** Refresh token é rotacionado e armazenado pelo adaptador.
- [ ] **CA-03:** Uma única renovação é executada para requisições concorrentes.
- [ ] **CA-04:** Falha definitiva limpa a sessão e pede confirmação do perfil.
