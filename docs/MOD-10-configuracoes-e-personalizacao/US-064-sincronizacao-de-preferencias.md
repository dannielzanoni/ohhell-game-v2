# PRD — US 064 — Sincronização de preferências
...
Esta especificação detalha exclusivamente **US 064 — Sincronização de preferências**. Ela deve ser implementada no mesmo aplicativo React/Vite, usando a experiência Mobile abaixo de 768 px e a experiência Desktop a partir de 768 px.
...
Entregar **Sincronização de preferências** de forma verificável, sem duplicar regras, estado, chamadas HTTP, sessão WebSocket ou persistência entre os layouts Mobile e Desktop.
...
- **RF-01:** Adaptador suporta evento local e `storage` entre abas.
- **RF-02:** Uma mudança não gera loop de eventos.

## 7. Critérios de aceite

- [ ] **CA-01:** Adaptador suporta evento local e `storage` entre abas.
- [ ] **CA-02:** Uma mudança não gera loop de eventos.
