...
const visualLayoutKeys = [...ptKeys, ...enKeys].filter((key) =>
  /(?:^|\.)appNameShort\d*$/i.test(key),
);
...
if (
  ...
  if (visualLayoutKeys.length) {
    console.error(`visual/layout-only translation keys found: ${visualLayoutKeys.join(', ')}`);
  }
